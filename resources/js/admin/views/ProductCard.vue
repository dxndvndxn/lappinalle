    ...
    <div class="admin-product-card">
        ...
        <div class="admin-product-card-desc">
            ...
            <div class="wrap-card-price">
                ...
                <input type="text" class="input-transp" @change="changeSaleProduct" v-model.trim="saleProduct" v-bind:disabled="priceProduct === null">
            </div>
...
<!--                            <button @click="addSize" class="btn-admin-purpp"><img src="../../../img/whiteplus.png" alt=""></button>-->
                    ...
                    <div class="wrap-newsize-stock">
                        ...
                        <small v-if="errorInput" class="small-invalid">Пожалуйста, укажите размер.</small>
            ...
            <div class="admin-photos">
                <form @submit.prevent="">
                    ...
                    <div class="uploaded-content">
                        ...
                        <img v-for="(imgUp, i) in uploadedImgs" :src="imgUp.img" alt="" v-bind:class="imgUp.active ? 'puprr-border' : 'unctive-blu-img'" @click="clickUploadedImg(i)" :key="i">
                    </div>
                    <div class="wrap-load">
                    ...
                    <button class="admin-btn-complete" @click="doFirstImg()">выбрать обложкой</button>
                ...
                <div class="admin-main-photo" v-if="mainImg || video">
                    ...
                    <img :src="mainImg" v-if="mainImg && !videoOrImg" class="admin-main-photo-selected" alt="">
                    <video :src="video" v-if="videoOrImg"></video>
                    <img src="../../../img/krest-btn.png" class="admin-delete-photo" alt="" @click="deleteUploadedImg()">
                </div>
            </div>
            <div class="new-functionality">
                <div class="related-products">
                    <h1 class="admin-h3">Бренды</h1>
                    <div class="wrap-related-products">
                        <input type="text" placeholder="Названия брендов" class="input-transp" disabled>
                        <button class="btn-admin-arrow" @click="activeBrands = !activeBrands" v-bind:class="activeBrands ? 'admin-btn-arrow' : 'admin-btn-arrow-pass'"></button>
                    </div>
                    <AdminCrumbs v-if="activeBrands" @addBrand="addBrand" v-bind:allBrands="brands"/>
                    <div class="wrap-size-grid" v-if="productBrand">
                        <span>
                            {{productBrand}}
                        </span>
                    </div>
                ...
                <div class="related-products">
                    <h1 class="admin-h3">Сопутствующие товары</h1>
                    <div class="wrap-related-products">
                        <input type="text" placeholder="№ товара" class="input-transp" disabled>
                        <button class="btn-admin-arrow" @click="activeAllIds = !activeAllIds" v-bind:class="activeAllIds ? 'admin-btn-arrow' : 'admin-btn-arrow-pass'"></button>
                    </div>
                    <AdminCrumbs v-if="activeAllIds" @addRelatedId="addRelatedId" v-bind:allIds="allIdsProducts"/>
                    <div class="wrap-size-grid" v-if="relatedProducts.length">
                        <span v-for="(related, i) in relatedProducts" :key="i" @dblclick="deleteRelatedProducts(related)">
                            {{related}}
                        </span>
                    </div>
                </div>
                <div class="related-products">
                    <h1 class="admin-h3">Размеры без скидки</h1>
                    <div class="wrap-related-products">
                        <input type="text" placeholder="Новый размер" class="input-transp" disabled>
                        <button class="btn-admin-arrow" @click="activeSizesWithoutSale = !activeSizesWithoutSale" v-bind:class="activeSizesWithoutSale ? 'admin-btn-arrow' : 'admin-btn-arrow-pass'"></button>
                    </div>
                    <AdminCrumbs v-if="activeSizesWithoutSale" @chooseWithoutSale="chooseWithoutSale" v-bind:sizesWithoutSale="getAllSizes"/>
                    <div class="wrap-size-grid" v-if="sizesWithNoSale.length">
                        <span v-for="(sz, i) in sizesWithNoSale" :key="i" @dblclick="deleteSizeWithoutSale(sz)">
                            {{sz}}
                        </span>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            selectSizeForStockUpdate(count, size, i) {
                this.newSize = i;
                this.presentSizes.forEach((el, lcI) => {
                    if (lcI === i) {
                        el.active = true;
                    }else{
                        el.active = false;
            ...
            addRelatedId(id){
                let findRelatedId = this.relatedProducts.find(localId => localId === id);
            ...
            deleteRelatedProducts(id) {
                this.relatedProducts = this.relatedProducts.filter(localId => localId !== id);
                this.updateProduct(this.$route.params.id, 'relatedId', this.relatedProducts.join(','));
            },

            // Добавляем товары без скидки
            chooseWithoutSale(sz){
                let findSize = this.sizesWithNoSale.find(size => size === sz);
            ...
            deleteSizeWithoutSale(sz){
                this.sizesWithNoSale = this.sizesWithNoSale.filter(size => size !== sz);
                this.updateProduct(this.$route.params.id, 'sizeWithoutSale', this.sizesWithNoSale.join(','));
            },

            addBrand(id){
                this.updateProduct(this.$route.params.id, 'newBrand', id);
                this.$store.dispatch('GetOneProduct', this.$route.params.id);
            },
            ...
            doFirstImg(){
                ...
                this.uploadedImgs.forEach(item => {
                    if (item.img !== this.mainImg) {
                        filtredImgStr.push(item.img)
                    }
                })
                filtredImgStr.unshift(this.mainImg)
                let joinedImgStr = filtredImgStr.join(', ') + ', ';

                this.updateProduct(this.$route.params.id, 'updateImgPaths', joinedImgStr)
                this.$store.dispatch('GetOneProduct', this.$route.params.id);
            }
        ...
        watch: {
            ...
            getOneProduct(newValue){
                ...
                this.productBrand = newValue[0].brands_name;
                this.brands = newValue.allBrands;
                this.uploadedImgs = [];
                this.allIdsProducts = newValue.allIds;
