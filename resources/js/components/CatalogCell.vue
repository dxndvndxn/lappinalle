<template>
    <!-- <div class="catalog-items" v-if="total !== null || catalogData !== null"> -->
        <div class="catalog-items" v-if="(Array.isArray(catalogData) && catalogData.length)">
        <div class="item" v-for="(item, i) in catalogData" :key="i" @mouseleave="hideElements(i)">
            <svg v-if="item.activeIconT" @click="item.activeTshirt = !item.activeTshirt" :class="item.activeTshirt ? 'fill-svg' : 'unfill-svg'" width="30" height="28" class="icon-cell t-shirt" viewBox="0 0 30 28" xmlns="http://www.w3.org/2000/svg">
...
<!--            </router-link>-->
    ...
    <!-- <p v-else-if="total > 0 && $route.name === 'gender'" class="catalog-error">
       По вашему запросу ничего не найдено.
    </p> -->
    <p v-else-if="Array.isArray(catalogData) && !catalogData.length" class="catalog-error">
       По вашему запросу ничего не найдено.
    </p>
</template>
...
<script>
    export default {
        ...
        methods:{
            async GoToProduct(id, sexAlias){
                console.log(sexAlias)
                this.$Progress.start();
                await this.$store.dispatch('getItemData', id)
                    .then((res) => {
                        if (res) {
                            if (Object.keys(this.$route.params).length) {
                                this.$router.push(`${this.$route.path}/item-${id}`).catch((e)=>{console.log(e)});
                            }else{
                                this.$router.push(`${sexAlias}/item-${id}`).catch((e)=>{console.log(e)});
                            }
                        }
                    })
            },
